...
class JTicketingOrderItem extends CMSObject
{
	...
	public function reValid($eventId)
	{
		...
		// Use tolerance for floating point precision issues with multiple ticket types
		$tolerance = 0.10; // Allow 10 cents difference due to rounding
		$amountDifference = abs($originalAmountPaid - round($this->amount_paid, 2));
		
		// Log for debugging
		Factory::getApplication()->getLogger()->debug('JTicketing: Order item amount validation attempt', [
		...
		{
			// Log for debugging
			Factory::getApplication()->getLogger()->error('JTicketing: Order item amount validation failed - significant difference', [
				'order_item_id' => $this->id,
				'type_id' => $this->type_id,
				'original_amount_paid' => $originalAmountPaid,
				'new_amount_paid' => $this->amount_paid,
		...
		// Log when tolerance is used
		if ($amountDifference > 0.001)
		{
			Factory::getApplication()->getLogger()->debug('JTicketing: Order item amount validation used tolerance', [
				'order_item_id' => $this->id,
				'type_id' => $this->type_id,
				'original_amount_paid' => $originalAmountPaid,
				'new_amount_paid' => $this->amount_paid,
