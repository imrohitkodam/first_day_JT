...
class JTicketingOrder extends CMSObject
{
	...
	private function check($ticketData, $action)
	{
		...
		// Debug logging for ticket-level validation
		Factory::getApplication()->getLogger()->debug('JTicketing Debug - Ticket Level Validation', [
			'ticket_id' => $ticketData->id,
			'ticket_title' => $ticketData->title,
			'max_ticket_per_order' => $ticketData->max_ticket_per_order ?? 'NOT_SET',
			'current_count' => $typeDeatails->count,
			'action' => $action,
			'global_limit' => $orderLimit,
			'will_use_global_limit' => (!isset($ticketData->max_ticket_per_order) || !$ticketData->max_ticket_per_order || $ticketData->max_ticket_per_order == 0) ? 'YES' : 'NO'
		]);

		// PRIORITY 1: Check ticket-level limit first (overrides global limit)
		...
		// PRIORITY 2: Check per transaction ticket buying limit (global limit as fallback)
		// Only apply global limit if ticket-level limit is not set or is 0
		if (!isset($ticketData->max_ticket_per_order) || !$ticketData->max_ticket_per_order || $ticketData->max_ticket_per_order == 0)
		{
			// Debug logging for global limit validation
			Factory::getApplication()->getLogger()->debug('JTicketing Debug - Global Limit Validation', [
				'ticket_id' => $ticketData->id,
				'ticket_title' => $ticketData->title,
				'current_tickets_count' => $this->ticketscount,
				'global_order_limit' => $orderLimit,
				'action' => $action,
				'will_check_global_limit' => ($this->ticketscount > (int) $orderLimit && $action == 'addItem' && $orderLimit !== '0') ? 'YES' : 'NO'
			]);
			
			// Only check global limit if ticket-level limit is not set
			// FIXED: Changed >= to > to allow exact limit amount
			if ($this->ticketscount > (int) $orderLimit && $action == 'addItem' && $orderLimit !== '0')
			{
				Factory::getApplication()->getLogger()->warning('JTicketing: Global ticket limit exceeded', [
					'ticket_id' => $ticketData->id,
					'current_tickets_count' => $this->ticketscount,
					'global_order_limit' => $orderLimit,
					'action' => $action
	...
	public function reValidateAmount()
	{
		...
		foreach ($OrderItems as $OrderItem)
		{
			...
			// Debug logging for order item validation
			Factory::getApplication()->getLogger()->debug('JTicketing Debug - Order Item Validation', [
				'order_id' => $this->id,
				'order_item_id' => $OrderItem->id,
				'type_id' => $OrderItem->type_id,
				'event_details_id' => $this->event_details_id
			]);

			if (!$odItem->reValid($this->event_details_id))
			{
				Factory::getApplication()->getLogger()->error('JTicketing: Order item reValid failed', [
					'order_id' => $this->id,
					'order_item_id' => $OrderItem->id,
					'type_id' => $OrderItem->type_id,
					'event_details_id' => $this->event_details_id
		...
		foreach ($ticketStat as $ticketId => $stat)
		{
			...
			// Debug logging for ticket availability check
			Factory::getApplication()->getLogger()->debug('JTicketing Debug - Ticket Availability Check', [
				'order_id' => $this->id,
				'ticket_id' => $ticketId,
				'available' => $available,
				'required_count' => $stat['count'],
				'unlimited_seats' => $ticket->unlimited_seats
			]);

			if ($available != COM_JTICKETING_CONSTANT_TICKET_TYPE_UNLIMITED && $available < $stat['count'])
			{
				Factory::getApplication()->getLogger()->error('JTicketing: Ticket availability exceeded', [
					'order_id' => $this->id,
					'ticket_id' => $ticketId,
					'available' => $available,
					'required_count' => $stat['count'],
		...
		// Debug logging for global limit validation in reValidateAmount
		Factory::getApplication()->getLogger()->debug('JTicketing Debug - reValidateAmount Global Limit Check', [
			'order_id' => $this->id,
			'current_tickets_count' => $this->ticketscount,
			'global_order_limit' => $orderLimit,
			'will_fail_validation' => ($this->ticketscount > (int) $orderLimit && $orderLimit != 0) ? 'YES' : 'NO'
		]);

		// Check per transaction ticket buying limit.
		if ($this->ticketscount > (int) $orderLimit && $orderLimit != 0)
		{
			Factory::getApplication()->getLogger()->warning('JTicketing: reValidateAmount - Global ticket limit exceeded', [
				'order_id' => $this->id,
				'current_tickets_count' => $this->ticketscount,
				'global_order_limit' => $orderLimit
			]);
		...
		/**
		 * Compare amounts with tolerance for floating point precision issues
		 * This fixes the issue where multiple ticket types cause validation failures
		 * due to tiny rounding differences in fee, tax, and coupon calculations
		 *
		 ...
		 */
		$tolerance = 0.10; // Allow 10 cents difference due to rounding
		$amountDifference = abs($this->amount - $oldAmount);
		
		// Always log the validation attempt for debugging
		Factory::getApplication()->getLogger()->info('JTicketing: Order amount validation attempt', [
			'order_id' => $this->id,
			'old_amount' => $oldAmount,
			'new_amount' => $this->amount,
			'difference' => $amountDifference,
			'tolerance' => $tolerance,
			'ticket_count' => $this->ticketscount,
			'fee' => $this->fee,
			'tax' => $this->order_tax,
		...
		{
			// Log significant amount differences for debugging
			Factory::getApplication()->getLogger()->error('JTicketing: Order amount validation failed - significant difference', [
				'order_id' => $this->id,
				'old_amount' => $oldAmount,
				'new_amount' => $this->amount,
				'difference' => $amountDifference,
		...
		// Log when tolerance is used to help monitor the fix
		if ($amountDifference > 0.001) // Only log if there's any meaningful difference
		{
			Factory::getApplication()->getLogger()->debug('JTicketing: Order amount validation used tolerance', [
				'order_id' => $this->id,
				'old_amount' => $oldAmount,
				'new_amount' => $this->amount,
				'difference' => $amountDifference,
